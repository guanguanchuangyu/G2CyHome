...
namespace G2CyHome.EntityConfiguration.Identity
{
    public partial class RoleClaimConfiguration : EntityTypeConfigurationBase<RoleClaim, int>
    {
        ...
        {
            builder.HasOne(rc => rc.Role).WithMany(r => r.RoleClaims).HasForeignKey(m => m.RoleId).IsRequired().HasConstraintName("FK_RoleClaim_RoleId");
            builder.HasIndex(m => m.RoleId).HasDatabaseName("IX_RoleClaim_RoleId");
