...
namespace G2CyHome.EntityConfiguration.Systems
{
    public partial class AuditEntityConfiguration : EntityTypeConfigurationBase<AuditEntity, Guid>
    {
        ...
        {
            builder.HasIndex(m => m.OperationId).HasDatabaseName("IX_AuditEntity_OperationId");
            builder.HasOne(m => m.Operation).WithMany(n => n.AuditEntities).HasForeignKey(m => m.OperationId).HasConstraintName("FK_Audit_OperationId");
